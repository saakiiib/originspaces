...
<div class="mb-2"><label class="form-label">Name *</label><input class="form-control" id="name" placeholder="e.g. Exterior"></div>
</form></div>
...
<div class="container-fluid"><div class="card"><div class="card-header"><h4>Gallery Categories</h4></div>
<div class="card-body"><div class="table-responsive"><table id="catTable" class="table table-bordered table-striped w-100"><thead><tr><th>Sl</th><th>Name</th><th>Slug</th><th>Images</th><th>Status</th><th>Action</th></tr></thead></table></div></div></div></div>
@endsection
@section('script')
<script>
$(function () {
    $.ajaxSetup({ headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') } });
    const t = $('#catTable').DataTable({ processing: true, serverSide: true, ajax: "{{ route('gallery-categories.index') }}",
    ...
    $('#saveBtn').click(function () {
        ...
        $.post(create ? "{{ route('gallery-categories.store') }}" : "{{ route('gallery-categories.update') }}", { id: $('#codeid').val(), name: $('#name').val() },
    ...
    $(document).on('click', '.editBtn', function () { $.get("{{ url('/admin/gallery-categories') }}/" + $(this).data('id') + '/edit', d => { $('#codeid').val(d.id); $('#name').val(d.name); $('#saveBtn').val('Update').html('Update'); $('#formBox').show(300); $('#newBtn').hide(); pagetop(); }); });
    $(document).on('change', '.toggle-status', function () { $.post("{{ route('gallery-categories.toggleStatus') }}", { id: $(this).data('id') }, d => { showSuccess(d.message); t.ajax.reload(null, false); }); });
});
</script>
@endsection
